...
class Controller(http.Controller):
	...
	def sale_fill_order_create_view(self, waybill_no=None):
		values = {
	...
	def sale_fill_order_create(self, **kwargs):
		...
		sale_shipping_no = request.env['sale.order'].sudo().search([('shipping_no', '=', kwargs.get('shipping_no'))])
		if sale_shipping_no:
			values = {
				'user_name': request.env.user.name,
	...
	def sale_portal_fill_order(self, order_id=None, user_name=None, shipping_no=None, error_message=None,**kwargs):
	...
	def sale_portal_fill_order_line(self, order_id=None, order_line_id=None, shipping_no=None, **kwargs):
		...
		if order_line_id:
			...
			sale_category_id, product_brand_id, product_material_id = product.sale_category_id.id,\
														product.brand_id.id, product.material_id.id
	...
	@http.route('/sale/portal/delete_order_line', type='http', auth='public', methods=['GET'], csrf=False)
	def sale_portal_delete_order_line(self, order_id, order_line_id,**kwargs):
		sale_order = request.env['sale.order'].browse(int(order_id))
		sale_order_line = sale_order.order_line.filtered(lambda l:l.id == int(order_line_id))
		sale_order_line.unlink()
		return request.redirect('/sale/portal/fill_order?%s'%url_encode({'order_id':order_id}))
	...
	def sale_portal_save_order(self, order_id=None, user_name=None, shipping_no=None, lines=None, **kwargs):
		try:
			...
			sale_order.sudo().write({'shipping_no':shipping_no})
			shipping_order = request.env['shipping.bill'].sudo().search([('name', '=', shipping_no)])
			if not shipping_order:
				shipping_order = request.env['shipping.bill'].sudo().create({
					'name': shipping_no,
				})
			shipping_order.write({
			...
			return request.redirect('/sale/portal/orders')
	...
	def sale_portal_orders(self, **kwargs):
		if request.env.user == request.env.ref('base.public_user'):
			redirect_url = '/sale/portal/orders'
			return request.redirect('/web/login?redirect=%s'%redirect_url)

		sale_orders = request.env['sale.order'].sudo().model_get_portal_orders()
